...
namespace RobotControllerApp
{
    ...
    public sealed partial class MainWindow : Window
    {
            ...
            try
            {
                ...
                while (!token.IsCancellationRequested && capture.IsOpened())
                {
                    if (capture.Read(mat) && !mat.Empty())
                    {
                        ...
                        {
                            // Cache Intel / second-camera frames for the Context Scan panel
                            _ = UpdateIntelScanFeedAsync(frameBytes);
                        }
        ...
        // ════════════════════════════════════════════════════════════════════════
        // ROBOT CONTROL PANEL — Show / Hide
        // ════════════════════════════════════════════════════════════════════════

        private void RobotControlPanelBtn_Click(object sender, RoutedEventArgs e)
        {
            if (RobotControlOverlay.Visibility == Visibility.Collapsed)
                OpenRobotControlPanel();
            else
                CloseRobotControlPanel();
        }

        private void OpenRobotControlPanel()
        {
            // Size the overlay height to match the window content area
            RobotControlOverlay.Visibility = Visibility.Visible;
            UpdateRobotControlPanelHeight();

            // Mirror Intel cam feed into the scan preview
            if (_latestIntelFrameBytes != null)
                _ = UpdateScanPreviewAsync(_latestIntelFrameBytes);
        }

        private void CloseRobotControlPanel_Click(object sender, RoutedEventArgs e) => CloseRobotControlPanel();
        private void RobotControlBackdrop_Tapped(object sender, Microsoft.UI.Xaml.Input.TappedRoutedEventArgs e) => CloseRobotControlPanel();

        private void CloseRobotControlPanel()
        {
            RobotControlOverlay.Visibility = Visibility.Collapsed;
        }

        private void UpdateRobotControlPanelHeight()
        {
            try
            {
                double h = RootGrid.ActualHeight;
                if (h > 0 && RobotControlPanel != null)
                    RobotControlPanel.Height = h;
            }
            catch { }
        }
        ...
        /// <summary>Current teleoperation enabled state (propagated to IK message filter).</summary>
        private bool _teleopEnabled = false;
        /// <summary>Follow-speed factor 1-100 (used to throttle IK command rate).</summary>
        private double _teleopSpeedPct = 30.0;

        private void TeleopToggle_Toggled(object sender, RoutedEventArgs e)
        {
            _teleopEnabled = TeleopToggle.IsOn;

            if (TeleopStatusLabel != null)
                TeleopStatusLabel.Text = _teleopEnabled
        ...
        private void TeleopSpeedSlider_ValueChanged(object sender,
        ...
        // Latest Intel frame cached for scan preview
        ...
        private async Task StartRecordingAsync()
        {
            try
            {
                WhisperStatusLabel.Text = "Initialising mic…";
                WhisperRecordDot.Fill = new SolidColorBrush(Microsoft.UI.Colors.OrangeRed);
                ...
                _isRecording = true;
                WhisperStatusLabel.Text = "Recording…";
                WhisperRecordDot.Fill = new SolidColorBrush(Windows.UI.Color.FromArgb(255, 255, 60, 60));
                WhisperRecordLabel.Text = "Stop";
                WhisperRecordIcon.Glyph = "\uE71A"; // Stop icon
                Log("[Voice] Recording started");
            }
            catch (Exception ex)
            {
                WhisperStatusLabel.Text = $"Mic error: {ex.Message}";
                WhisperRecordDot.Fill = new SolidColorBrush(Microsoft.UI.Colors.Gray);
        ...
        private async Task StopRecordingAsync()
        {
            try
            {
                WhisperStatusLabel.Text = "Stopping…";
                ...
                WhisperRecordDot.Fill = new SolidColorBrush(Windows.UI.Color.FromArgb(255, 80, 200, 80));
                WhisperRecordLabel.Text = "Record";
                WhisperRecordIcon.Glyph = "\uE720"; // Mic icon

                WhisperStatusLabel.Text = "Transcribing…";
            ...
            {
                WhisperStatusLabel.Text = $"Stop error: {ex.Message}";
        ...
        private async Task TranscribeAudioAsync()
        {
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    DispatcherQueue.TryEnqueue(() =>
                    {
                        WhisperTranscriptBox.Text = text.Trim();
                        WhisperStatusLabel.Text = $"Done ({text.Trim().Length} chars)";
                        WhisperRecordDot.Fill = new SolidColorBrush(Windows.UI.Color.FromArgb(255, 0, 200, 100));
                    });
                ...
                else
                {
                    DispatcherQueue.TryEnqueue(() =>
                    {
                        WhisperStatusLabel.Text = $"API error {(int)response.StatusCode}";
                        WhisperRecordDot.Fill = new SolidColorBrush(Microsoft.UI.Colors.OrangeRed);
                    });
            ...
            catch (Exception ex)
            {
                DispatcherQueue.TryEnqueue(() =>
                {
                    WhisperStatusLabel.Text = $"Error: {ex.Message}";
                    WhisperRecordDot.Fill = new SolidColorBrush(Microsoft.UI.Colors.OrangeRed);
                });
        ...
        {
            string text = WhisperTranscriptBox?.Text?.Trim() ?? "";
            ...
            WhisperStatusLabel.Text = "Sent ✓";
        }

        // ════════════════════════════════════════════════════════════════════════
        // PANEL 3 — CONTEXT SCAN (Intel Camera + Gemini bounding boxes)
        // ════════════════════════════════════════════════════════════════════════
        ...
        private async void ContextScanBtn_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                ScanStatusLabel.Text = "No Orange API key — see Settings";
                return;
            }
            if (frame == null)
            {
                ScanStatusLabel.Text = "No camera frame available";
                return;
            }

            ScanStatusLabel.Text = "Scanning…";
            ScanBtnIcon.Glyph = "\uE768"; // Spinning-ish
            ScanPanelBtn.IsEnabled = false;
            _scanDetectedLabels.Clear();
            ScanDetectedList.ItemsSource = _scanDetectedLabels;
            ...
            try
            {
                ...
                await AnalyzeSceneWithCallbackAsync(frame,
                    ...
                    {
                        ScanStatusLabel.Text = $"{count} object{(count == 1 ? "" : "s")} detected";
                        ScanBtnIcon.Glyph = "\uE773";
                        ScanPanelBtn.IsEnabled = true;
                    }));
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Calls the Orange/Gemini bounding-box pipeline and fires callbacks per detected label.
        /// Reuses the same API path as AnalyzeSceneAsync but triggers callbacks for panel UI instead.
        /// </summary>
        private async Task AnalyzeSceneWithCallbackAsync(byte[] frameBytes,
            Action<string>? onLabel, Action<int>? onDone)
        {
            if (Interlocked.CompareExchange(ref _analyzeInProgressFlag, 1, 0) == 1)
            {
                ScanStatusLabel.Text = "Scan already in progress";
                return;
            }
            try
            {
                ...
                if (!string.IsNullOrEmpty(messageContent))
                {
                    ...
                    if (s >= 0 && eIdx > s)
                    {
                        ...
                        if (itemsDoc.RootElement.ValueKind == JsonValueKind.Array)
                        {
                            ...
                            using var sourceMat = OpenCvSharp.Cv2.ImDecode(frameBytes, OpenCvSharp.ImreadModes.Color);
                            double imgW = sourceMat.Width;
                            double imgH = sourceMat.Height;

                            DispatcherQueue.TryEnqueue(() => ScanBoundingBoxCanvas?.Children.Clear());

                            foreach (var item in itemsDoc.RootElement.EnumerateArray())
                            {
                                string label = item.TryGetProperty("label", out var lp) ? lp.GetString() ?? "?" : "?";
                                if (!item.TryGetProperty("box_2d", out var box) || box.GetArrayLength() < 4) continue;

                                int yn = box[0].GetInt32(); int xn = box[1].GetInt32();
                                int yx = box[2].GetInt32(); int xx = box[3].GetInt32();

                                // Normalise 0-1000 to canvas coordinates (160px tall preview)
                                double previewH = 160.0, previewW = 388.0;
                                double cx = xn / 1000.0 * previewW;
                                double cy = yn / 1000.0 * previewH;
                                double cw = (xx - xn) / 1000.0 * previewW;
                                double ch = (yx - yn) / 1000.0 * previewH;

                                DispatcherQueue.TryEnqueue(() =>
                                {
                                    if (ScanBoundingBoxCanvas == null) return;
                                    var rect = new Microsoft.UI.Xaml.Shapes.Rectangle
                                    {
                                        Width = Math.Max(cw, 4),
                                        Height = Math.Max(ch, 4),
                                        Stroke = new SolidColorBrush(Windows.UI.Color.FromArgb(220, 0, 229, 255)),
                                        StrokeThickness = 1.5,
                                        Fill = new SolidColorBrush(Windows.UI.Color.FromArgb(30, 0, 229, 255))
                                    };
                                    Microsoft.UI.Xaml.Controls.Canvas.SetLeft(rect, cx);
                                    Microsoft.UI.Xaml.Controls.Canvas.SetTop(rect, cy);
                                    ScanBoundingBoxCanvas.Children.Add(rect);

                                    var tb = new TextBlock
                                    {
                                        Text = label,
                                        FontSize = 9,
                                        Foreground = new SolidColorBrush(Windows.UI.Color.FromArgb(255, 0, 229, 255)),
                                        Background = new SolidColorBrush(Windows.UI.Color.FromArgb(160, 0, 0, 0))
                                    };
                                    Microsoft.UI.Xaml.Controls.Canvas.SetLeft(tb, cx + 2);
                                    Microsoft.UI.Xaml.Controls.Canvas.SetTop(tb, Math.Max(cy - 14, 0));
                                    ScanBoundingBoxCanvas.Children.Add(tb);
                                });
        ...
        private async Task UpdateIntelScanFeedAsync(byte[] frameBytes)
        {
            ...
            // If panel is open, update the live preview
            if (RobotControlOverlay?.Visibility == Visibility.Visible)
                await UpdateScanPreviewAsync(frameBytes);
        }
        ...
        private void SaehQuadrant_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (SaehSelectedLabel != null)
            {
                ...
                SaehSelectedLabel.Text = desc;
            }

            // Highlight the active quadrant and reset others
            ...
            _ = _broadcastServer.BroadcastAsync("sisSaeh",
                JsonSerializer.Serialize(new { category = tag }));
        }
        ...
        {
            // Broadcast a simulate command to the 3D viewer (ar.html / Unity)
            // The viewer triggers its IK simulation animation loop
            int robotIdx = TeleopR2Radio?.IsChecked == true ? 1 : 0;
            _ = _broadcastServer.BroadcastAsync("simulateIK",
                JsonSerializer.Serialize(new
                {
                    robotIdx,
                    saeh = _saehSelected,
            ...
            if (SimulateStatusLabel != null)
                SimulateStatusLabel.Text = $"Simulation broadcast → Robot {robotIdx + 1}";
        ...
        private async void ExecuteTrajectoryBtn_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (!bridge.IsConnected)
            {
                if (ExecuteStatusLabel != null)
                    ExecuteStatusLabel.Text = $"Robot {robotIdx + 1} not connected";
                return;
            }
            ...
            try
            {
                // Broadcast an executeTrajectory signal to the 3D viewer /ar.html
                // The viewer will send ik_joints with the final target angles back through
                // the WebSocket → MainWindow → Bridge pipeline
                await _broadcastServer.BroadcastAsync("executeTrajectory",
                    JsonSerializer.Serialize(new
                    {
                        robotIdx,
                        saeh = _saehSelected,
                ...
                if (ExecuteStatusLabel != null)
                    ExecuteStatusLabel.Text = $"Trajectory sent to Robot {robotIdx + 1} ✓";
            }
            catch (Exception ex)
            {
                Log($"[Execute] Error: {ex.Message}");
                if (ExecuteStatusLabel != null)
                    ExecuteStatusLabel.Text = $"Error: {ex.Message}";
            }
            finally
            {
